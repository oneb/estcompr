...
from assert_helpers import is_pos_int, is_good_handle
...
def alg1(fh, A, eps, logheader=True, logcomment=''):
    ...
    text_log.debug('')
    text_log.debug('starting run')
    text_log.debug('logcomment: {}'.format(comment))
    text_log.debug(fh.name)
    text_log.debug('A={A}, eps={eps}'.format(A=A, eps=eps))
    text_log.debug('n={n:,}, N={N:,}, l_0={l_0:,}'.format(n=n, N=N, l_0=l_0))
    text_log.debug('B={B:.4f}, repeats={r:,}, ceil(s)={s:,}'.format(
        B=B, r=repeats, s=slice_count))
    text_log.debug(
        'sampling {:,} out of {:,} substrings ({:.4%}); {:,} ({:.4%}) per repetition for {:,} repetitions'
        .format(tot_ss_qd, N, tot_ss_qd_frac, slice_count, it_ss_qd_frac,
                repeats))
    text_log.debug(
        'sampling {:,} out of {:,} characters ({:.4%}); {:,} ({:.4%}) per repetition for {:,} repetitions'
        .format(tot_chars_qd, n, tot_chars_qd_frac, it_chars_qd,
                it_chars_qd_frac, repeats))
    ...
    text_log.debug('runtime {} seconds'.format(runtime.total_seconds()))
    text_log.debug('completed with {}'.format(ret))
    ...
    data = OrderedDict([
        ...
        ('comment', comment), ('filename', fn), ('A', A), ('eps', eps),
        ('invEps', 1 / eps), ('n (file size)', n), ('B', B), ('l_0', l_0),
        ('ceil(s)', slice_count), ('repeats', repeats),
        ...
        ('PL estimate (rounded)', round(ret)), ('PL frac estimate', ret / n),
        ('PL lower bound', low), ('PL upper bound', high),
        ('PL frac lower bound', low / n), ('PL frac upper bound', high / n),
